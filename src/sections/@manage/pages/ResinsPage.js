...
const ResinsPage = () => {
  ...
  const showToastMessage = () => {
    if (!id) toast.success('Resina agregada con éxito!', {
      position: toast.POSITION.TOP_RIGHT
    });
    else toast.success('Resina actualizada con éxito!', {
      position: toast.POSITION.TOP_RIGHT
    });
  };
                        ...
                        <TableRow hover key={id} tabIndex={-1} role="checkbox">
                          ...
                          <TableCell align="left">
                            <ButtonSwitch checked={active} inputProps={{ 'aria-label': 'ant design' }} onClick={
                              async () => {
                                if (active) (
                                  showToastMessageStatus('error', 'Resina desactivada')
                                )
                                else (
                                  showToastMessageStatus('success', 'Resina activada')
                                )
        ...
        <DialogContent dividers>
          <Stack spacing={3} sx={{ minWidth: 550 }}>
            ...
            <FormControl sx={{ width: '100%' }} error={!!errors?.weight}>
              <InputLabel htmlFor="weight-adornment-amount">Peso de la resina</InputLabel>
              <Controller
                name="weight"
                control={control}
                defaultValue=""
                rules={{
                  required: 'El peso de la resina es requerido',
                  min: {
                    value: 500,
                    message: 'El peso de la resina debe ser mayor o igual a 500'
                  },
                  max: {
                    value: 100000,
                    message: 'El peso de la resina debe ser menor o igual a 100000'
                  }
                }}
                render={({ field: { onChange, onBlur, value, } }) => (
                  <OutlinedInput
