...
const SearchFilaments = ({handleAddFilament}) => {
    ...
    const getDataAutoComplete = (searchTerm) => {
        ...
        axios.get(`/api/filaments/s/${searchTerm}`, { signal })
            .then((res) => {
    ...
    return (
        <>
            <Autocomplete
                ...
                clearOnEscape
                blurOnSelect
                renderInput={(params) => <TextField {...params} label="Nombre del filamento" />}
            />
        </>
    )
}

export default SearchFilaments